   ...
   const char * caFile)
{
   ...
   if (err != LDAP_SUCCESS)
   {
      ...
      return [NSString stringWithUTF8String:ldap_err2string(err)];
   };
   ...
   while(entry)
   {
     ...
     // NSLog(@"      dn: %s", ldap_get_dn(ld, entry));
      ...
      while(attribute)
      {
         ...
         {
            for(i = 0; vals[i]; i++)
                usrDetails = [[usrDetails stringByAppendingString:[NSString stringWithUTF8String:attribute]] stringByAppendingString:[NSString stringWithFormat:@":%@\n",[NSString stringWithUTF8String:vals[i]->bv_val]]];
